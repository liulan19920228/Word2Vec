...
def run(text_path='text8', vocab_path='vocab.txt', output_path='vectors.txt'):
    ...
    learning_rate = 0.02
    min_count = 100
    vocab = Vocab(text_path,vocab_path,min_count)
    ...
    while epoch <20:
        ...
        for i in range(3, len(index)-3):
            ...
            #context_set = index[i-window_size:pos]+index[pos+1:i+window_size+1]
        ...
        #time2=time.time()
            if i % 100000 == 0:
                learning_rate = max(learning_rate - 0.25/1000,0000, 0.0001*0.25)
                #loss = 0
                #for j, t in classifier:
                #    if t == 0:
                #        t = -1
    ...
    vocab_given_file = open(vocab_path, 'r').read()
...
run(text_path='text8', vocab_path='vocab.txt', output_path='vectors.txt')
